...
private struct ProxyLatencyMeasurementPlan {
    let groupsToRefresh: [ProxyGroup]
...
/// Lightweight ref-counted presence tracker.
///
/// Wraps a `@Published Set` so that overlapping begin/end pairs from concurrent
/// callers never drop the key prematurely.  The generic `SetStorage` closure gives
/// the caller control over *which* published set is mutated.
...
/// Two-level ref-counted presence: `[Outer: Set<Inner>]`.
struct NestedRefCountedPresence<Outer: Hashable, Inner: Hashable> {
    ...
    mutating func end(outer: Outer, inner: Inner, from published: inout [Outer: Set<Inner>]) {
        ...
        guard current > 1 else {
            ...
            if self.counts[outer]?.isEmpty == true { self.counts.removeValue(forKey: outer) }

            published[outer]?.remove(inner)
            if published[outer]?.isEmpty == true { published.removeValue(forKey: outer) }
            return
        }
...
extension AppSession {
    ...
    /// O(1) group lookup backed by a lazily-rebuilt index.
    ...
    /// Rebuild the dictionary index from `proxyGroups`.
    /// Called once per measurement pass or when groups are refreshed.
    ...
    private func refreshResolvedGroupLatencies(startingFrom rootGroups: [ProxyGroup]) async {
        ...
        // Pre-compute direct nodes for whole-group types (used later in defer blocks).
        ...
        rootGroups: [ProxyGroup]) -> ProxyLatencyMeasurementPlan
    {
        ...
        return ProxyLatencyMeasurementPlan(
            groupsToRefresh: groupsToRefresh,
    ...
    /// Each whole-group measurement finishes independently — the first group
    /// to complete ends its loading spinner without waiting for slower groups.
            ...
            func enqueueNextGroup() {
                ...
                taskGroup.addTask { [group, cachedDirectNodes] in
                    await self.measureAndFinalizeWholeGroupLatency(
                        group, cachedDirectNodes: cachedDirectNodes)
                }
            }
    ...
    /// Measure a single whole-group, refresh proxy state, and immediately end
    /// that group's loading — fully self-contained per group.
        ...
        cachedDirectNodes: [String]?) async
    {
        ...
        // Refresh group state so url-test `now` picks up the fastest node.
        await self.refreshProxyGroups()

        // Clear pending keys and loading for this group immediately.
    ...
    private func beginGroupLatencyPending(groupName: String, delayKey: String) {
        self.pendingDelayKeyRefCount.begin(
            outer: groupName, inner: delayKey,
